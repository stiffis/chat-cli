...
void save_chat_history(const std::string &filename,
...
void draw_interface(WINDOW *contacts_win, WINDOW *chat_win, WINDOW *status_win,
                    ...
                    const std::string &current_message, int selected_contact,
                    bool in_chat) {
    // Dibujar contactos
    werase(contacts_win);
    ...
    for (size_t i = 0; i < contacts.size(); ++i) {
        if (i == selected_contact) {
            wattron(contacts_win,
                    A_REVERSE); // Resalta el contacto seleccionado
        }
    ...
    werase(chat_win);
    box(chat_win, 0, 0);
    mvwprintw(chat_win, 1, 1, "Chat with %s",
              contacts[selected_contact].c_str());
    ...
    if (in_chat) {
        mvwprintw(chat_win, row + 1, 1, "Type your message: %s",
    ...
    if (in_chat) {
        mvwprintw(status_win, 0, 1,
                  "Status: Typing a message (Press Esc to go back)");
    } else {
        mvwprintw(status_win, 0, 1,
                  "Status: Selecting a contact (Press Enter to chat)");
    }
    wrefresh(status_win);
}
...
int main() {
    ...
    // Crear ventanas
    WINDOW *contacts_win =
        newwin(rows - 1, contacts_width, 0, 0); // Ventana de contactos
    WINDOW *chat_win =
        newwin(rows - 1, chat_width, 0, contacts_width); // Ventana de chat
    WINDOW *status_win = newwin(1, cols, rows - 1, 0);   // Barra de estado

    // Lista de contactos
    std::vector<std::string> contacts = {"Alice", "Bob", "Charlie", "Diana",
                                         "Eve"};
    int selected_contact = 0;

    // Historial de mensajes y mensaje actual
    std::vector<std::string> chat_history =
        load_chat_history(contacts[selected_contact] + ".txt");
    ...
    draw_interface(contacts_win, chat_win, status_win, contacts, chat_history,
                   current_message, selected_contact, in_chat);
    ...
    while ((ch = getch()) != 'q' || in_chat) {
        if (!in_chat) {
            ...
            if (ch == 27) { // Salir del modo de chat (Esc)
                in_chat = false;
                save_chat_history(contacts[selected_contact] + ".txt",
                                  chat_history);
            } else if (ch == '\n') { // Enviar mensaje
                ...
                if (!current_message.empty()) {
                    current_message.pop_back();
                       ...
                       ch <= 126) { // Agregar carácter (caracteres imprimibles)
                current_message.push_back(ch);
            }
        }
        // Redibujar interfaz
        draw_interface(contacts_win, chat_win, status_win, contacts,
                       chat_history, current_message, selected_contact,
                       in_chat);
    }
